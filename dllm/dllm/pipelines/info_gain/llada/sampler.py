...
from dllm.pipelines.info_gain.core import (
    compute_entropy, expand_kv, trim_kv, pad_logits,
...
def _info_gain_select(
    ...
    n_cand, pos_temp, tok_temp, mask_id,
    *, past_key_values=None, prefix_len=0,
    ...
    result = generate_candidates(logits, x, mask_allowed,
                                 block_start, block_end, k, n_cand,
                                 tok_temp, pos_temp)
    actions, x0s, conf_base, valid, _ = result

    def _make(sel, x0):
        tr = x.new_zeros(1, T, dtype=torch.bool); tr[0, sel] = True
        return torch.where(tr, x0, x), compute_entropy(logits)[0, sel].sum().item(), None

    # Trivial early returns
    if actions is None:
        nv = valid.shape[0]
        if nv == 0: return x.clone(), 0.0, None
        if nv <= k: return _make(valid, x0s)
        _, ti = torch.topk(conf_base[0], k); return _make(ti, x0s)
    if len(actions) <= 1:
    ...
    with torch.no_grad():
        if dual_cache and past_key_values is not None:
            ...
            nl = _shift(model(xb[:, block_start:block_end], attention_mask=at,
                              past_key_values=ep, use_cache=False, replace_position=rp).logits)
            ...
            ep = expand_kv(past_key_values, nc)
            nl = _shift(model(xb[:, prefix_len:], attention_mask=attention_mask,
                              past_key_values=ep, use_cache=False).logits)
            if suppress_fn: suppress_fn(nl)
    ...
    def sample(
        ...
        **kwargs,
        ...
        C = {f: kwargs.get(f, getattr(config, f)) for f in (
            'steps', 'max_new_tokens', 'max_length', 'block_size', 'temperature',
            'return_dict', 'right_shift_logits', 'suppress_tokens',
            'begin_suppress_tokens', 'use_cache', 'threshold',
            'candidate_number', 'position_temperature', 'remasking',
        )}
        ...
        if mnt is None: mnt = ml - mpl
        T = int(ml)

        x = torch.full((B, T), self.tokenizer.eos_token_id, dtype=torch.long, device=self.model.device)
        attn = torch.zeros((B, T), dtype=torch.long, device=self.model.device)
        for i, p in enumerate(inputs_list):
            x[i, :len(p)] = p; x[i, len(p):min(len(p)+mnt, T)] = mask_id
            attn[i, :min(len(p)+mnt, T)] = 1

        histories = [x.clone()] if C['return_dict'] else None
        bs = C['block_size']; nb = math.ceil(mnt / bs); spb = math.ceil(C['steps'] / nb)
        ...
        def suppress(l):
            for lst in (C['suppress_tokens'], C['begin_suppress_tokens']):
                if lst:
                    for tid in lst: l[:, :, tid] = -torch.inf

        def prep(model_out):
            l = model_out.logits; suppress(l)
            if C['right_shift_logits']: l = torch.cat([l[:, :1], l[:, :-1]], dim=1)
            return l

        ig = dict(n_cand=C['candidate_number'], pos_temp=C['position_temperature'],
                  tok_temp=C['temperature'], mask_id=mask_id,
                  right_shift_logits=C['right_shift_logits'], suppress_fn=suppress)
        ...
        for b in range(nb):
            ...
            eff = ntt.size(1); cached = None

            def _mask():
                m = torch.zeros_like(x, dtype=torch.bool); m[:, s:e] = x[:, s:e] == mask_id; return m

            def _step(logits_full, ki, is_last, *, pkv=None, pl=0, dc=False, rp=None):
                nonlocal x, cached
                ma = _mask()
                if not ma.any(): return False
                if is_last:                                           # last step → fill all
                    x0 = torch.argmax(add_gumbel_noise(logits_full, C['temperature']), -1)
                    x = torch.where(ma, x0, x); cached = None
                    histories and histories.append(x.clone()); return False
                bp = self._bypass(logits_full, x, ma, s, e, ki, C['threshold'])
                if bp is not None:
                    x = bp; cached = None
                    histories and histories.append(x.clone()); return True
                x, _, cached = _info_gain_select(self.model, x, logits_full, ma, ki,
                    past_key_values=pkv, prefix_len=pl, dual_cache=dc,
                    replace_position=rp, block_start=s, block_end=e,
                    attention_mask=attn, **ig)
                histories and histories.append(x.clone()); return True

            def _remaining(): return int((x[:, s:e] == mask_id).sum().item())

            # ── no cache ────────────────────────────────────────────────
            if use_cache is None:
                for i in range(eff):
                    if not _remaining(): break
                    ki = ntt[0, i].item()
                    if ki <= 0: continue
                    if cached is not None: logits = cached; cached = None
                    else: logits = prep(self.model(x, attention_mask=attn))
                    if not _step(logits, ki, ki >= _remaining()): break
                continue
            ...
            if use_cache == "prefix":
                ...
                _step(l0, ntt[0, 0].item(), ntt[0, 0].item() >= _remaining(), pkv=pkv, pl=s)
                pkv = trim_kv(pkv, s)
                for i in range(1, eff):
                    if not _remaining(): break
                    ki = ntt[0, i].item()
                    if ki <= 0: continue
                    if cached is not None: lfp = cached; cached = None
                    else:
                        ls = prep(self.model(x[:, s:], attention_mask=attn,
                                             past_key_values=pkv, use_cache=False))
                        lfp = ls.new_zeros(1, T, ls.shape[-1]); lfp[:, s:] = ls
                    if not _step(lfp, ki, ki >= _remaining(), pkv=pkv, pl=s): break
                continue
            ...
            if use_cache == "dual":
                ...
                _step(l0, ntt[0, 0].item(), ntt[0, 0].item() >= _remaining(),
                      pkv=pkv, dc=True, rp=rp)
                for i in range(1, eff):
                    if not _remaining(): break
                    ki = ntt[0, i].item()
                    if ki <= 0: continue
                    if cached is not None: lf = cached; cached = None
                    else:
                        lb = prep(self.model(x[:, s:e], attention_mask=attn,
                                             past_key_values=pkv, use_cache=True,
                                             replace_position=rp))
                        lf = pad_logits(lb, T, s, x.device)
                    if not _step(lf, ki, ki >= _remaining(),
                                 pkv=pkv, dc=True, rp=rp): break

        return x if not C['return_dict'] else BaseSamplerOutput(sequences=x, histories=histories)

    @staticmethod
    def _bypass(logits, x, ma, s, e, k, threshold):
        if threshold is None: return None
        p = F.softmax(logits.float(), dim=-1)
        t1, x0 = p.max(-1); x0 = torch.where(ma, x0, x)
        hc = (t1[:, s:e][0] >= threshold) & ma[:, s:e][0]
        if not hc.any(): return None
        idx = torch.where(hc)[0]
        if len(idx) > k: _, tk = torch.topk(t1[:, s:e][0, idx], k); idx = idx[tk]
        xo = x.clone(); xo[0, s + idx] = x0[0, s + idx]; return xo
