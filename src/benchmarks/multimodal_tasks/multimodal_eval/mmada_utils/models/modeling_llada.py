...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, max_replace_pos: Optional[int] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.autocast(q.device.type, enabled=False):
            ...
            # Use max_replace_pos if provided (for dual cache mode)
            rope_len = max_replace_pos if max_replace_pos is not None else key_len
...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        # KV cache is now supported for LLaDA models
